...
echo
echo "======================================================"
echo " Scenario 3: External Client via LoadBalancer"
echo "======================================================"
echo

# =============================================================================
# CONTEXT SETUP — Establish gateway cluster context
# =============================================================================
# Use CTX_CLUSTER1 env var if set (from installation-steps/001), otherwise
# let the user pick from available contexts.
if [ -n "${CTX_CLUSTER1:-}" ]; then
    GATEWAY_CONTEXT="${CTX_CLUSTER1}"
    info "Using gateway context from \$CTX_CLUSTER1: ${GATEWAY_CONTEXT}"
else
    warn "\$CTX_CLUSTER1 is not set. Available contexts:"
    ...
    for idx in "${!ALL_CONTEXTS[@]}"; do
        ...
        if [[ "${CTX}" == "${CURRENT_CONTEXT}" ]]; then
            echo "  $((idx+1))) ${CTX}  ← current"
        else
            echo "  $((idx+1))) ${CTX}"
        fi
    done
    echo
    read -r -p "Select context for gateway cluster (Cluster 1) [enter number]: " GW_CHOICE
    GW_CHOICE="${GW_CHOICE:-1}"
    GATEWAY_CONTEXT="${ALL_CONTEXTS[$((GW_CHOICE-1))]}"
fi
info "Gateway context: ${GATEWAY_CONTEXT}"
echo
...
echo "─────────────────────────────────────────────────────"
echo
info "Scenario 3 requires a LoadBalancer so the external NLB hop is included in latency."
info "Patching EnterpriseAgentgatewayParameters → service.spec.type=LoadBalancer …"
kubectl --context "${GATEWAY_CONTEXT}" patch enterpriseagentgatewayparameters agentgateway-config -n "${AGW_NAMESPACE}" \
    --type=merge \
...
for i in $(seq 1 24); do
    GATEWAY_IP=$(kubectl --context "${GATEWAY_CONTEXT}" get svc agentgateway-proxy -n "${AGW_NAMESPACE}" \
...
if [[ "${CLIENT_MODE}" == "2" ]]; then
    ...
    # Default to CTX_CLUSTER2 if set, otherwise prompt for selection.
    # -------------------------------------------------------------------------
    if [ -n "${CTX_CLUSTER2:-}" ]; then
        CLIENT_CONTEXT="${CTX_CLUSTER2}"
        info "Using client context from \$CTX_CLUSTER2: ${CLIENT_CONTEXT}"
    else
        info "Gateway context: ${GATEWAY_CONTEXT}"
        echo
        ...
        for idx in "${!ALL_CONTEXTS[@]}"; do
            CTX="${ALL_CONTEXTS[$idx]}"
            if [[ "${CTX}" == "${GATEWAY_CONTEXT}" ]]; then
                echo "  $((idx+1))) ${CTX}  ← gateway cluster"
            else
                echo "  $((idx+1))) ${CTX}"
            fi
        done
        echo
        read -r -p "Select context for client cluster (Cluster 2) [enter number]: " CTX_CHOICE
        CTX_CHOICE="${CTX_CHOICE:-1}"
        CLIENT_CONTEXT="${ALL_CONTEXTS[$((CTX_CHOICE-1))]}"
    fi

    if [[ "${CLIENT_CONTEXT}" == "${GATEWAY_CONTEXT}" ]]; then
