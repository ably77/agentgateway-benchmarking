#!/usr/bin/env bash
# Tear down all Scenario 1c resources.
set -euo pipefail
...
read -r -p "This will delete all Scenario 1c resources. Continue? [y/N]: " CONFIRM
...
echo
echo "If the loadgen client was deployed to a separate cluster context, run:"
echo "  kubectl --context <client-context> delete -f ${SCRIPT_DIR}/k8s/agent-deployment.yaml --ignore-not-found"
echo "  kubectl --context <client-context> delete namespace ${AGENT_NAMESPACE} --ignore-not-found"
