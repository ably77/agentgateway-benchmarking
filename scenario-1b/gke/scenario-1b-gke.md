# Scenario 1b - Dedicated Proxies: GKE Cluster Setup

## Architecture
- Single cluster, single region, single availability zone
- 3 isolated node groups (one per logical tier)
- Dedicated proxies (agentgateway-proxy-mcp for MCP traffic, agentgateway-proxy-llm for LLM traffic)
- Single client

```
Node Group 1 (Zone A): ns=agent-1             → Client-1
Node Group 2 (Zone A): ns=agentgateway-system  → Agentgateway (MCP proxy + LLM proxy)
...
```bash
GKE_CLUSTER_NAME="ly-gke-benchmark-scenario1b"
...
```bash
GKE_NODE_POOL_NAME="agent-scenario1b"
...
gcloud container clusters resize ${GKE_CLUSTER_NAME} \
  ...
  --node-pool agent-scenario1b --num-nodes 1 --quiet

# Scale to zero
gcloud container clusters resize ${GKE_CLUSTER_NAME} \
  --zone ${POOL_ZONE} --project ${GKE_PROJECT} \
  --node-pool agent-scenario1b --num-nodes 0 --quiet
```

---

## Node Group 2 — Agentgateway (Dedicated Proxies: MCP + LLM)
> Namespace: `agentgateway-system`
>
> Both `agentgateway-proxy-mcp` and `agentgateway-proxy-llm` run on this node group.

```bash
GKE_NODE_POOL_NAME="agentgateway-scenario1b"
POOL_ZONE="${GKE_CLUSTER_ZONE}"
POOL_MACHINE_TYPE="n2-standard-4"
POOL_NUM_NODES="1"
POOL_MIN_NODES="1"
...
gcloud container clusters resize ${GKE_CLUSTER_NAME} \
  --zone ${POOL_ZONE} --project ${GKE_PROJECT} \
  --node-pool agentgateway-scenario1b --num-nodes 1 --quiet

# Scale to zero
gcloud container clusters resize ${GKE_CLUSTER_NAME} \
  --zone ${POOL_ZONE} --project ${GKE_PROJECT} \
  --node-pool agentgateway-scenario1b --num-nodes 0 --quiet
```
...
```bash
GKE_NODE_POOL_NAME="capability-scenario1b"
POOL_ZONE="${GKE_CLUSTER_ZONE}"
POOL_MACHINE_TYPE="n2-standard-4"
POOL_NUM_NODES="4"
POOL_MIN_NODES="4"
...
gcloud container clusters resize ${GKE_CLUSTER_NAME} \
  --zone ${POOL_ZONE} --project ${GKE_PROJECT} \
  --node-pool capability-scenario1b --num-nodes 4 --quiet

# Scale to zero
gcloud container clusters resize ${GKE_CLUSTER_NAME} \
  --zone ${POOL_ZONE} --project ${GKE_PROJECT} \
  --node-pool capability-scenario1b --num-nodes 0 --quiet
```
