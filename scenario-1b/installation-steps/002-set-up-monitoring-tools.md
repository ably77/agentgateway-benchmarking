...
- Configure Prometheus to scrape both Agentgateway proxies
...
Add PodMonitor for scraping metrics from both agentgateway proxies.
The selector uses `matchExpressions` to match both `agentgateway-proxy-mcp` and `agentgateway-proxy-llm` pods so that the shared Grafana dashboard works without modification — all proxy metrics flow into the same Prometheus data source.
```bash
kubectl apply -f- <<EOF
